...
REFERENCE_AUDIO_TEXT_COLUMN = "reference_audio_text"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "reference_audio_path and reference_audio_text lists"
        ),
    )
    parser.add_argument(
        "--output_dir",
    ...
    parser.add_argument(
        ...
        default=None,
        help="CosyVoice2 language tag to prefix text: zh/en/ja/yue/ko",
    )
    parser.add_argument(
        "--system_prompt",
        default=DEFAULT_SYSTEM_PROMPT,
        help=(
            "CosyVoice3 system prompt placed before each reference transcript "
...
def validate_args(args: argparse.Namespace) -> None:
    if args.model_version == "cosy3" and args.lang:
        raise ValueError("--lang is only valid with --model_version cosy2")
    if args.model_version == "cosy3" and not args.system_prompt.endswith(
        "<|endofprompt|>"
    ):
        raise ValueError("CosyVoice3 --system_prompt must end with <|endofprompt|>")
    model_dir = Path(args.model_path).expanduser().resolve()
    expected_config = model_dir / MODEL_CONFIG_NAMES[args.model_version]
    ...
    for record_index, record in enumerate(payload):
        ...
        if len(reference_audio_paths) != len(reference_audio_texts):
            raise ValueError(
                ...
                "reference_audio_text must have the same length"
            )
        conversations.append(
            InputConversation(
                ...
                reference_audio_texts=reference_audio_texts,
            )
        )
...
def candidate_tsv_row(candidate: ExpandedCandidate) -> Dict[str, str]:
    return {
        ...
        REFERENCE_AUDIO_TEXT_COLUMN: candidate.reference_audio_text,
...
def child_command(args: argparse.Namespace, chunk_input_path: Path, chunk_dir: Path) -> List[str]:
    ...
    if args.model_version == "cosy3":
        command.extend(["--system_prompt", args.system_prompt])
    if args.overwrite:
            ...
            failure_rows.append(
                {
                    ...
                    "ref_audio_text": row.get("ref_audio_text", ""),
                    "error": row.get("error", ""),
                }
            )
    ...
    for conversation in conversations:
        ...
        for text_index, text in enumerate(conversation.texts):
            ...
            for reference_index, reference_audio_path in enumerate(
                ...
                reference_audio_texts.append(
                    conversation.reference_audio_texts[reference_index]
                )
            outputs.append(
                {
                    "text": text,
                    ...
                    "reference_audio_text": reference_audio_texts,
                }
            )
