...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--llm_backend",
        choices=["native", "trtllm"],
        default="native",
        help="LLM backend for stage-1 token generation (default: native)",
    )
    parser.add_argument(
        "--trt_engine_dir",
        default=None,
        help="TensorRT-LLM engine directory (required for --llm_backend trtllm)",
    )
    parser.add_argument(
        "--trt_max_input_len",
        type=int,
        default=512,
        help="TensorRT-LLM max input length (default: 512)",
    )
    parser.add_argument(
        "--trt_max_output_len",
        type=int,
        default=2048,
        help="TensorRT-LLM max generated token length (default: 2048)",
    )
    parser.add_argument(
        "--trt_kv_cache_free_gpu_memory_fraction",
        type=float,
        default=0.6,
        help="TensorRT-LLM KV cache free GPU memory fraction (default: 0.6)",
    )
    parser.add_argument(
        "--trt_temperature",
        type=float,
        default=0.8,
        help="TensorRT-LLM sampling temperature (default: 0.8)",
    )
    parser.add_argument(
        "--trt_top_k",
        type=int,
        default=50,
        help="TensorRT-LLM top-k sampling (default: 50)",
    )
    parser.add_argument(
        "--trt_top_p",
        type=float,
        default=0.95,
        help="TensorRT-LLM top-p sampling (default: 0.95)",
    )
    parser.add_argument(
        "--save_workers",
        type=int,
...
def main() -> None:
    ...
    if args.llm_backend == "trtllm" and not args.trt_engine_dir:
        raise ValueError("--trt_engine_dir is required when --llm_backend is trtllm")
    if args.trt_max_input_len <= 0 or args.trt_max_output_len <= 0:
        raise ValueError("--trt_max_input_len and --trt_max_output_len must be positive")
    if args.trt_top_k <= 0:
        raise ValueError("--trt_top_k must be positive")
    if not 0.0 < args.trt_top_p <= 1.0:
        raise ValueError("--trt_top_p must be in (0, 1]")
    if args.trt_temperature <= 0.0:
        raise ValueError("--trt_temperature must be positive")
    if not 0.0 < args.trt_kv_cache_free_gpu_memory_fraction <= 1.0:
        raise ValueError("--trt_kv_cache_free_gpu_memory_fraction must be in (0, 1]")
    ...
    runner = StagedBatchInferenceRunner(
        ...
        trt_max_output_len=args.trt_max_output_len,
        trt_kv_cache_free_gpu_memory_fraction=args.trt_kv_cache_free_gpu_memory_fraction,
        trt_temperature=args.trt_temperature,
        trt_top_k=args.trt_top_k,
        trt_top_p=args.trt_top_p,
    )
