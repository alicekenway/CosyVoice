...
from frontend_batch import ZeroShotBatchPreparer
...
MODEL_CLASS_NAMES = {"cosy2": "CosyVoice2", "cosy3": "CosyVoice3"}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--input_tsv",
        required=True,
        help="TSV with text, reference_audio_path, and reference_audio_text columns",
    )
    parser.add_argument(
        "--output_dir",
        required=True,
...
def validate_model_args(args: argparse.Namespace) -> Path:
...
def build_failure_row(row: TsvInputRow, error_message: str) -> Dict[str, str]:
    failure_row = {
        ...
        "ref_audio_text": row.ref_audio_text,
        "error": error_message,
    }
...
def main() -> None:
    ...
    expected_class_name = MODEL_CLASS_NAMES[args.model_version]
    actual_class_name = cosyvoice.__class__.__name__
