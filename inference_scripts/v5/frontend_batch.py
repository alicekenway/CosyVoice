...
def collate_segment_inputs(segment_inputs: List[SegmentInput]) -> Dict[str, torch.Tensor]:
    ...
    prompt_text_list = [
        item.model_input["prompt_text"].squeeze(0).to(torch.int32)
        for item in segment_inputs
    ]
    prompt_text_len_list = [
        item.model_input["prompt_text_len"].squeeze(0).to(torch.int32)
        for item in segment_inputs
    ]

    llm_prompt_speech_token_list = [
        item.model_input["llm_prompt_speech_token"].squeeze(0).to(torch.int32)
        for item in segment_inputs
    ]
    llm_prompt_speech_token_len_list = [
        item.model_input["llm_prompt_speech_token_len"].squeeze(0).to(torch.int32)
        for item in segment_inputs
    ]
    ...
    prompt_text = pad_sequence(prompt_text_list, batch_first=True, padding_value=0)
    prompt_text_len = torch.tensor(
        [length.item() for length in prompt_text_len_list], dtype=torch.int32
    )

    llm_prompt_speech_token = pad_sequence(
        llm_prompt_speech_token_list,
        batch_first=True,
        padding_value=0,
    )
    llm_prompt_speech_token_len = torch.tensor(
        [length.item() for length in llm_prompt_speech_token_len_list],
        dtype=torch.int32,
    )
...
class ZeroShotBatchPreparer:
    ...
    def _get_prompt_features(self, ref_audio_path: str) -> PromptFeatures:
        ...
        prompt_features = PromptFeatures(
            ...
            llm_prompt_speech_token_len=flow_prompt_speech_token_len,
            flow_prompt_speech_token=flow_prompt_speech_token,
            flow_prompt_speech_token_len=flow_prompt_speech_token_len,
            prompt_speech_feat=prompt_speech_feat,
            prompt_speech_feat_len=prompt_speech_feat_len,
