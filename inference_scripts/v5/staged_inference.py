...
class StagedBatchInferenceRunner:
    ...
    def _run_llm_stage_batch(
        ...
        prompt_text = batch_inputs["prompt_text"].to(device)
        prompt_text_len = batch_inputs["prompt_text_len"].to(device)
        prompt_speech_token = batch_inputs["llm_prompt_speech_token"].to(device)
        prompt_speech_token_len = batch_inputs["llm_prompt_speech_token_len"].to(device)
        batch_size = text.shape[0]

        combined_text_tensors: List[torch.Tensor] = []
        for batch_index in range(batch_size):
            this_prompt_len = int(prompt_text_len[batch_index].item())
            this_text_len = int(text_len[batch_index].item())
            combined_text_tensors.append(
                torch.cat(
                    [
                        prompt_text[batch_index, :this_prompt_len],
                        text[batch_index, :this_text_len],
                    ]
                )
            )
        combined_text = pad_sequence(
            combined_text_tensors, batch_first=True, padding_value=0
        )
        combined_text_len = prompt_text_len + text_len
        ...
        if is_transformer_backend:
            ...
            text_emb = llm_module.text_embedding(combined_text.to(torch.long))
            text_encoded, text_encoded_len = llm_module.encode(
                text_emb, combined_text_len
            )
            ...
            text_encoded = llm_module.llm.model.model.embed_tokens(
                combined_text.to(torch.long)
            )
            text_encoded_len = combined_text_len.to(torch.long)
            llm_embedding = None
        else:
            raise RuntimeError(
        ...
        for batch_index in range(batch_size):
            ...
            this_prompt_speech_len = int(
                prompt_speech_token_len[batch_index].item()
            )
            if this_prompt_speech_len:
                parts.append(
                    llm_module.speech_embedding(
                        prompt_speech_token[
                            batch_index : batch_index + 1,
                            :this_prompt_speech_len,
                        ].to(torch.long)
                    ).squeeze(0)
                )
    ...
    def _run_flow_hift_stage_batch(
        ...
        h, mel_total_len = self._encode_flow_tokens(
            flow_module=flow_module,
            token_emb=token_emb,
            token_total_len=token_total_len,
    ...
    def _encode_flow_tokens(
        ...
        """Encode prompt and generated tokens for supported flow architectures."""
        device = token_emb.device
        if hasattr(flow_module, "pre_lookahead_layer"):
            h = flow_module.pre_lookahead_layer(token_emb)
            h = h.repeat_interleave(int(flow_module.token_mel_ratio), dim=1)
            mel_new_len = gen_token_len * int(flow_module.token_mel_ratio)
            mel_total_len = prompt_feat_len + mel_new_len
            return h[:, : int(mel_total_len.max().item())], mel_total_len

        if hasattr(flow_module, "length_regulator"):
            h, _ = flow_module.encoder(token_emb, token_total_len)
            ...
            return h, mel_total_len

        if hasattr(flow_module, "encoder"):
            h, _ = flow_module.encoder(
                token_emb, token_total_len, streaming=False
            )
        ...
        raise RuntimeError(
            f"Unsupported flow module: {flow_module.__class__.__name__}"
        )
