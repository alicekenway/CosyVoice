...
def resolve_columns(fieldnames: Sequence[str]) -> Dict[str, str]:
    ...
    for column_name in fieldnames:
        ...
        if normalized_name in {
            ...
            column_mapping["ref_audio_text"] = column_name
...
def load_rows(input_tsv_path: Path) -> List["TsvInputRow"]:
    ...
    with input_tsv_path.open("r", encoding="utf-8", newline="") as tsv_file:
        ...
        if not {"text", "ref_audio", "ref_audio_text"}.issubset(column_mapping):
            raise ValueError(
                "Input TSV must contain columns for text, reference audio path, "
                "and reference audio text. "
...
def failure_fieldnames(include_input_id: bool = False) -> List[str]:
    ...
    if include_input_id:
        fieldnames.append("id")
    fieldnames.extend(["text", "ref_audio", "ref_audio_text", "error"])
    return fieldnames
